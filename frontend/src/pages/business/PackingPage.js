...
import { useGetExpensesQuery } from '../../features/inventory/expenseSlice';
...
const PackingPage = () => {
  ...
  // Keep formData BEFORE any useMemo that references it
  ...
  useEffect(() => {
    if (selectedPurchase) {
      ...
      const originalQtyInLeast = convertToLeast(parseFloat(selectedPurchase.quantity || 0), baseUnit);
      const perLeastPrice = parseFloat(selectedPurchase.price || 0) / (originalQtyInLeast || 1);
      ...
      const totalExpense = purchaseExpenses.reduce(
      ...
      const relatedPurchases = purchases.filter((p) => allRelatedPurchaseIDs.includes(String(p.purchaseID)));
      ...
      const totalExpenseForThisPurchase =
        totalPriceOfAllPurchasesInExpense > 0
          ? (selectedPurchasePrice / totalPriceOfAllPurchasesInExpense) * totalExpense
          : 0;

      const expensePerLeastUnit =
        totalExpenseForThisPurchase / (convertToLeast(parseFloat(selectedPurchase.quantity || 0), baseUnit) || 1);
      expensePerLeastUnitRef.current = expensePerLeastUnit;

      const defaultQty = product.quantity || 0;
      const defaultQtyLeast = convertToLeast(parseFloat(defaultQty || 0), baseUnit);
  ...
  const availableCategories = useMemo(() => {
    ...
    const nameMatched = products.filter(
      (p) => p.name?.toLowerCase().includes(formData.productName.toLowerCase())
    );
    const uniqCatIds = [...new Set(nameMatched.map((p) => p.category_id))];
    return categories.filter((c) => uniqCatIds.includes(c.id));
  }, [products, categories, formData.productName]);

  const selectedCategoryId = useMemo(() => {
    const c = categories.find((c) => c.name === formData.categoryName);
    return c?.id || null;
  ...
  const matchingFinancials = useMemo(() => {
    ...
    const filtered = products.filter((p) => {
      const nameOk = p.name?.toLowerCase().includes(formData.productName.toLowerCase());
      const catOk = !selectedCategoryId || p.category_id === selectedCategoryId;
      return nameOk && catOk;
    });
  ...
  }, [products, units, formData.productName, selectedCategoryId]);

  const availableQuantities = useMemo(() => {
    if (!formData.packingUnit) return matchingFinancials.map((f) => f.quantity);
    return matchingFinancials.filter((f) => f.unitName === formData.packingUnit).map((f) => f.quantity);
  ...
  const handleChange = (e) => {
    ...
    // If category changes, reset selection-dependent fields
    if (name === 'categoryName') {
      updatedForm.quantity = '';
      updatedForm.packingUnit = '';
    }
    ...
    if (selectedPurchase) {
      ...
      const usedQtyInLeast = !isNaN(qty) && !isNaN(packs) ? convertToLeast(qty, selectedUnit) * packs : 0;
      ...
      const perLeastPrice =
        parseFloat(selectedPurchase.price || 0) /
        (convertToLeast(parseFloat(selectedPurchase.quantity || 0), baseUnit) || 1);

      const currentQtyInLeast = convertToLeast(qty || 0, selectedUnit);

      // Expense proportional share
      const purchaseExpenses = expenses.filter(
        (exp) =>
          Array.isArray(exp.purchaseIDs) &&
  ...
  const getBarcodeFromProduct = (productName, categoryID, brandID, unitID, quantity) => {
    ...
    const matchedProduct = nameMatchedProducts.find(
      (p) =>
        (!categoryID || p.category_id === categoryID) &&
        p.brand_id === brandID &&
        p.unit_id === unitID &&
        parseFloat(p.quantity) === parseFloat(String(quantity))
    );
  ...
  const handleSubmit = async (e) => {
    ...
    const barcodeArr = getBarcodeFromProduct(
      ...
      selectedCategoryId, // use user-chosen category
    ...
    const packingData = {
      ...
      categoryID: selectedCategoryId || selectedPurchase.categoryID, // store chosen/fallback
    ...
    // Update remaining purchase quantity
    ...
    const availableInLeast = convertToLeast(parseFloat(selectedPurchase.availableQuantity || 0), baseUnit);
        ...
        {selectedPurchase && (
          ...
          >
            ...
            <div>
              ...
              >
                <option value="">
                  {formData.productName ? '-- Select Category --' : 'Select a purchase first'}
                </option>
            ...
            {[
              ...
              ([label, name, handler = handleChange, readOnly = false, bg = '', type = 'number'], idx) => (
                <div key={idx}>
                  <label className="block text-sm font-medium">{label}</label>
                  <input
                    type={type}
